...
async function main() {
  ...
  const { data: tree } = await octokit.rest.git.createTree({ owner, repo, base_tree: commit.tree.sha, tree: [{ path: 'server/routes.ts', mode: '100644', type: 'blob', sha: blob.sha }] });
  const { data: nc } = await octokit.rest.git.createCommit({ owner, repo, message: 'Use new OAuth client with /oauth/callback redirect URI', tree: tree.sha, parents: [ref.object.sha] });
