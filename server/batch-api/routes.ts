...
export function registerBatchApiRoutes(app: Express) {
  ...
  app.post('/api/admin/direct-translate', async (req: Request, res: Response) => {
    try {
      if (!(await isAdmin(req))) {
        return res.status(401).json({ error: 'Unauthorized - Admin access required' });
      }
      ...
      const result = await runDirectTranslation(limit);
      res.json(result);
    } catch (err: any) {
      console.error('[Direct Translate] Error:', err);
      res.status(500).json({ error: err.message || 'Failed to run direct translation' });
    }
  });

  app.get('/api/admin/direct-translate/stats', async (req: Request, res: Response) => {
    try {
      if (!(await isAdmin(req))) {
        return res.status(401).json({ error: 'Unauthorized - Admin access required' });
      }
      ...
      console.error('[Direct Translate] Error fetching stats:', err);
      res.status(500).json({ error: err.message || 'Failed to fetch translation stats' });
    }
  });
